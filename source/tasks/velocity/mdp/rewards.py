...
def feet_lateral_separation_l2(
  ...
  """Penalize feet that are closer than a minimum lateral separation.

  The first configured site is assumed to be the left foot and the second the
  right foot. Their relative position is expressed in the root-link frame so
  the measured lateral separation remains meaningful as the robot turns.
  """
  asset: Entity = env.scene[asset_cfg.name]
  if len(asset_cfg.site_ids) != 2:
    raise ValueError(
      "feet_lateral_separation_l2 requires exactly two sites ordered as "
      "(left, right)."
    )

  foot_pos_w = asset.data.site_pos_w[:, asset_cfg.site_ids, :]
  left_to_right_w = foot_pos_w[:, 0, :] - foot_pos_w[:, 1, :]
  left_to_right_b = quat_apply_inverse(
    asset.data.root_link_quat_w, left_to_right_w
  )
  lateral_width = left_to_right_b[:, 1]

  normalized_violation = torch.clamp(
    (min_width - lateral_width) / min_width, min=0.0
  )
  cost = torch.square(normalized_violation)
  ...
  env.extras["log"]["Metrics/foot_separation_mean"] = torch.mean(lateral_width)
  return cost
